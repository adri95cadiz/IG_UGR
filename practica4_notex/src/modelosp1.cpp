 ...
 ******************************************************************************/
 ...
 Tetraedro::Tetraedro(float size ){
 	...
 	//Vertices
 	texturas.push_back(_vertex2f(1, 1));
 	vertices.push_back(_vertex3f(0, (size*(sqrt(6)/3))/2, 0 )); //Punta del tetraedro.
 	//Vertices de la base.
 	texturas.push_back(_vertex2f(0, 1));
 	vertices.push_back(_vertex3f(-size/2, -(size*(sqrt(6)/3))/2, -size/3 ));
 	texturas.push_back(_vertex2f(1, 0));
 	vertices.push_back(_vertex3f(size/2, -(size*(sqrt(6)/3))/2, -size/3 ));
 	texturas.push_back(_vertex2f(0, 0));
 ...
 Cornetto::Cornetto(float height, float rad, int n_caras){
 	...
 	this->generar_normales();
 	this->calcular_coord_textura(n_div, 2);
 }
 ...
 Donut::Donut(float rad, float thick, int n_caras){
 	...
 	this->generar_normales();
	this->calcular_coord_textura(n_div, n_div);
 }

 Esfera::Esfera() {}

 Esfera::Esfera(float radio, unsigned int anillos, unsigned int sectores)
 {	
 		float giro = 180/(anillos);
 		int i;

 		vertices.push_back(_vertex3f(0, radio, 0)); //Inicio del giro.

 		//Vertices del primer giro. 	
 		for(i = 0; i<anillos; i++)
 			vertices.push_back(rotar_afin(vertices[i], _vertex3f(0, 0, 0 ) , giro));

	    this->generar_por_revolucion(sectores);
 ...
 Plano::Plano(float width, float height)
 {
 	texturas.push_back(_vertex2f(1, 1));
 	vertices.push_back(_vertex3f(width/2, height/2, 0)); 
 	texturas.push_back(_vertex2f(0, 1));
 	vertices.push_back(_vertex3f(-width/2, height/2, 0));
 	texturas.push_back(_vertex2f(1, 0));
 	vertices.push_back(_vertex3f(width/2, -height/2, 0));
